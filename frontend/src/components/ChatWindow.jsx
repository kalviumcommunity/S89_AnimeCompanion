// frontend/src/components/ChatWindow.jsx (UPDATED: Added Enter/Shift+Enter Logic)
...
function ChatWindow({ 
    sidebarOpen: sidebarOpenProp = true, 
    setSidebarOpen: setSidebarOpenProp = () => {},
    setCurrentView: setCurrentViewProp = () => {} // Passed from App.jsx
}) {
    ...
    const handleKeyDown = (e) => {
        // If Enter is pressed
        if (e.key === 'Enter') {
            // Check if Shift key is NOT pressed (for sending)
            if (!e.shiftKey) {
                e.preventDefault(); // Prevent default (newline in textarea)
                handleSend(e);      // Trigger the send function
            }
    ...
    <div className={`app-container ${sidebarOpen ? 'sidebar-open' : 'sidebar-collapsed'}`}>
        ...
        {isAuthenticated && (
            <Sidebar
                ...
                setCurrentView={setCurrentViewProp} // Passing setCurrentView to Sidebar
            />
        )}
            ...
            <div className="chat-input-area-wrapper">
                <form onSubmit={handleSend} className="chat-input-form">
                    <textarea
                        ...
                        onKeyDown={handleKeyDown} // 🎯 ADDED: Keyboard handler for Enter/Shift+Enter
